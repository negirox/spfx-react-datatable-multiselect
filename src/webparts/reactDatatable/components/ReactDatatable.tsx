...
import { Dropdown, IDropdownOption, IDropdownStyles, Link, Persona, PersonaSize, Text } from 'office-ui-fabric-react';
...
const dropdownStyles: Partial<IDropdownStyles> = { dropdown: { width: "auto" } };
export default class ReactDatatable extends React.Component<IReactDatatableProps, IReactDatatableState> {
    ...
    if (fields.length) {
      ...
      listItems = listItems && listItems.map(item => ({
        id: item.Id, ...fields.reduce((ob, f) => {
          ob[f.key] = item[f.key] ? this.formatColumnValue(item[f.key], f.fieldType,f['typeAsString']) : '-';
          return ob;
        }, {})
      }));
  ...
  public formatColumnValue(value: any, type: string,typeAsString:string) {
    if (!value) {
      return value;
    }
    switch (type) {
      case 'SP.FieldDateTime':
        value = value != (undefined || null) ? new Date(value).toUTCString() : value;
        break;
      ...
      case 'SP.FieldMultiLineText':
        if(typeAsString=="Thumbnail"){
          let imgURL=JSON.parse(value);
          value= <Persona
          imageUrl={imgURL['serverRelativeUrl']}
          text={imgURL['fileName']}
          size={PersonaSize.size32}
          imageAlt={imgURL['fileName']}
          styles={{ primaryText: { fontSize: '12px' } }}
      />;
         // value=<img src={imgURL['serverRelativeUrl']} title={imgURL['fileName']} style={{width:30, height:30, maxWidth:30}}/>;
        }
        else
                ...
                {list && fields && fields.length ?
                  <div>
                    <Grid container className={styles.dataTableUtilities}>
                      ...
                      enableSearching
                          ? columns.map(x=>{
                            var arrayofItems=filteredItems.map(colsItems=>{return colsItems[x.fieldName];});
                            return (<td>{this.CustomDropDown(arrayofItems)}</td>);
                          })
                          : <></>
                      } 
                      </tr>
  ...
  private CustomDropDown(items:any[]):React.ReactElement{
    let options:IDropdownOption[]=new Array<IDropdownOption>();
    var uItems=[];
    items.forEach(x=>{
      if(x.props){
        if(x.props.loginName){
          x=x.props.loginName;
        }
      }
      if(uItems.indexOf(x)==-1){
  ...
  private customHeaderForGrid(columns:any[],items:any[]):IColumn[]{
    let options:IColumn[]=new Array<IColumn>();
    columns.forEach(x=>{
      var arrayofItems=items.map(colsItems=>{return colsItems[x.fieldName];});
      var obj= {
        key: x,
        name: 'Name',
        fieldName: x,
